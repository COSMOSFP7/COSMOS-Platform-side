...
///printf("COMPARA %s - %s ", v.stringVal, rhs.v.stringVal);
	...
	*/
	void assertMachineTypes() const
	{
		...
		assert(sizeof(v.timeVal.hh) + sizeof(v.timeVal.mm) + sizeof(v.timeVal.ss) == 3);
	...
	*/
	unsigned char* serialize(unsigned char* mem_, size_t* calc_ = 0) const
	{
		...
		{
			if(!calc_) memcpy(mem_ + offs, &v.timeVal.hh, sizeof(v.timeVal.hh));
			offs += sizeof(v.timeVal.hh);
			if(!calc_) memcpy(mem_ + offs, &v.timeVal.mm, sizeof(v.timeVal.mm));
			offs += sizeof(v.timeVal.mm);
			if(!calc_) memcpy(mem_ + offs, &v.timeVal.ss, sizeof(v.timeVal.ss));
			offs += sizeof(v.timeVal.ss);
		}
	...
	*/
        TValue* deserialize(const unsigned char* _mem, size_t* count_ = 0)
	{
		...
		{
			memcpy(&v.timeVal.hh, _mem + offs, sizeof(v.timeVal.hh));
			offs += sizeof(v.timeVal.hh);
			memcpy(&v.timeVal.mm, _mem + offs, sizeof(v.timeVal.mm));
			offs += sizeof(v.timeVal.mm);
			memcpy(&v.timeVal.ss, _mem + offs, sizeof(v.timeVal.ss));
			offs += sizeof(v.timeVal.ss);
		}
	...
	const char* asString() const
	{
		...
		if(t == t_time)	
		{ 
			sprintf(fmt, "%02d:%02d:%02d/t", (int)v.timeVal.hh, 
									(int)v.timeVal.mm,
									(int)v.timeVal.ss);
			return fmt;
		}
...
*/
	...
	const char* asStringRaw() const
	{
		...
		if(t == t_time)	
		{ 
			sprintf(fmt, "%02d:%02d:%02d", (int)v.timeVal.hh, 
									(int)v.timeVal.mm,
									(int)v.timeVal.ss);
			return fmt;
		}
